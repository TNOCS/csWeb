module csComp.Services {
    ...
    export class LayerService {
        ...
        transformations: { [key: string]: ITransformation };
        ...
        constructor(
            ...
            // add transformations
            this.transformations["kriging"] = new KrigingInterpolationTransformation();
                ...
                (callback) => {
                    ...
                    if (this.layerSources.hasOwnProperty(layerSource)) {
                        ...
                        layer.layerSource.addLayer(layer, (l) => {
                            ...
                            this.updateMapFilter(layer.group);
                ...
                layer.transformations.forEach((transformation: IAppliedTransformation) => {
                    if (this.transformations[transformation.id] !== undefined) {
                        if (transformation.enabled)
                            this.transformations[transformation.id].apply(layer, transformation.options);
                    }
                });
        ...
        */
        ...
        public initLayer(group: ProjectGroup, layer: ProjectLayer, layerIds?: string[]) {
            ...
            if (layer.transformations === undefined) layer.transformations = [];
        ...
        public updateMapFilter(group: ProjectGroup) {
            if (group.filters.length === 0)
                group.filterResult = this.project.features.filter((f: IFeature) => f.layer.group.id === group.id);
