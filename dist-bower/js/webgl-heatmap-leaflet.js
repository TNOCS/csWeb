...
L.TileLayer.WebGLHeatMap = L.Class.extend({
        ...
        if (dataLen) {
            for (var i = 0; i < dataLen; i++) {
                ...
                heatmap.addPoint(
                    Math.floor(point.x),
                    Math.floor(point.y),
                    dataVal.length === 4 ? dataVal[3] : this.scale(latlng),
                    dataVal[2]);
            }
            heatmap.update();
            heatmap.display();
    ...
    scale: function(latlng, size) {
        ...
        var lngRadius = (size ? size : this.options.size / 40075017) * 360 / Math.cos(L.LatLng.DEG_TO_RAD * latlng.lat);
    ...
    /**
    * Add a data point, optionally supplying its radius in meters.
    */
    addDataPoint: function(lat, lon, value, size) {
    	if (size)
        	this.data.push([lat, lon, value, size]);
        else
        	this.data.push([lat, lon, value]);
    },
